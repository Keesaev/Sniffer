...
QString Network::getFullData() {
  QString s =
      ...
      "\n\tTime to live: " +
      QString::number(static_cast<int>(ipHeader.ip_ttl)) +
      "\n\tProtocol: " + QString::number(getProtocol()) + tr("(") +
      getProtocolName() + tr(")") +
...
void Network::deserializeHeader(const u_char *bytes) {
  ...
  string str(reinterpret_cast<const char *>(bytes),
...
int Network::getProtocol() { return ipHeader.ip_p; }
...
int Network::getHeaderSize() { return (((ipHeader).ip_vhl) & 0x0f) * 4; }

QString Network::getSourceIp() { return getAddress(ipHeader.ip_src); }

QString Network::getDestIp() { return getAddress(ipHeader.ip_dst); }
...
QString Network::getDiffServ() {
  ...
  if (res != diffServ.end()) s += " (DSCP: " + res->second + ")";
